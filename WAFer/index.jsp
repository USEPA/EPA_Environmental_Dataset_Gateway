...
// metadata/FRS , http://www.mymetadata.com:8080/public =>  http://www.mymetadata.com:8080/public/metadata/FRS
            ...
            String makeLengthLimitedName(String id, String name)
                ...
                if (id.length() + name.length() > limit) {
                    if (id.length() < limit) {
                        nameTry = name.substring(0, limit - id.length());
						if (nameTry.substring(nameTry.length() - 1, nameTry.length()).equals("%")) { // if nameTry ended with %
							nameTry = name.substring(0, limit - id.length() + 2);
						}
						if (nameTry.substring(nameTry.length() - 2, nameTry.length() - 1).equals("%")) {// if nameTry ended with %*
							nameTry = name.substring(0, limit - id.length() + 1);	
						}							
						name = nameTry;
					}
					else
