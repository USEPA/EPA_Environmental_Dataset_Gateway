...
private static String SAVE_SEARCH_TERM = "GPT_SEARCH_TERM";
private static String GPT_RESOURCE = "GPT_RESOURCE";
...
public void save(String name, String restCriteria, User user) 
  ...
  try {
	 ...
	 * @param sterm
	 */
	public void saveOrUpdateSearchTerm(String sterm) {
		try {
			int count = countSearchTerm(sterm);
			if (count == 0) {
				createSearchTerm(sterm);
			} else if (count > 0) {
				updateSearchTermCnt(sterm);
			}
	 ...
	 * @throws SearchException
	 */
	public void createSearchTerm(String sterm) throws SearchException {

		PreparedStatement pStmt = null;
		Connection connection = null;
		try {
			String sql = " INSERT INTO " + SAVE_SEARCH_TERM + " (STERM, NUM_OF_OCCURANCES) " + " VALUES(?,?)";
	 ...
	 * @throws SearchException
	 */
	public int countSearchTerm(String sterm) throws SearchException {
		...
		try {
			...
			String sql = "SELECT NUM_OF_OCCURANCES FROM " + SAVE_SEARCH_TERM + " WHERE STERM = ? ";
	 ...
	 * @throws SearchException
	 */
	public void updateSearchTermCnt(String sterm) throws SearchException {

		PreparedStatement pStmt = null;
		Connection connection = null;
		try {

			String sql = " UPDATE " + SAVE_SEARCH_TERM + " SET NUM_OF_OCCURANCES = ? WHERE STERM = ? ";
			int recCount = countSearchTerm(sterm);
			
			recCount = recCount + 1;
			connection = this.getConnection();
			pStmt = connection.prepareStatement(sql);
			pStmt.setInt(1, recCount);
			pStmt.setString(2, sterm);
			pStmt.execute();
 ...
 * @throws SearchException
 */
public String getDocUUID(String uuid)
	    throws SearchException {
	  Connection connection = null;
	  PreparedStatement ps = null;
	  ResultSet rs = null;
