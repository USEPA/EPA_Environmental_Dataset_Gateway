...
/**
 * Builds the collection of bind-able links associated with a document or
 * resource.
 */
public class ResourceLinkBuilder {
    ...
    /**
     * Builds a set of links associated with a rest based ArcGIS map, image or
     * globe service.
     *
     * @param xRecord the underlying CSW record
     * @param record the search result record
    ...
    /**
     * Builds a generic Open link for the resource URL associated with the
     * record.
     *
     * @param xRecord the underlying CSW record
     * @param record the search result record
  ...
  if (serviceType.equals("ags")) {
    ...
    private boolean showThumbnail() {
        @SuppressWarnings("unchecked")
        Map<String, String> extraArgsMap = (Map<String, String>) this.getRequestContext().getObjectMap().get(
                RestQueryServlet.EXTRA_REST_ARGS_MAP);
        if (extraArgsMap == null) {
            return true;
        }
     ...
     * <br/>Only urls beginning with www. http:// https:// ftp:// ftps:// will
     * be returned.
     *
    ...
    protected void determineResourceUrl(SearchXslRecord xRecord,
        ...
        for (int i = 0; i < schemeVals.size(); i++) {
            if (i == 6) {
                break; // Record up to six resource links
            }
            String url = schemeVals.get(i);
            ...
            try {
                label = org.apache.commons.lang3.StringEscapeUtils.unescapeHtml4(label.replace("?", ""));
                label = java.net.URLDecoder.decode(label, java.nio.charset.StandardCharsets.UTF_8.name());
