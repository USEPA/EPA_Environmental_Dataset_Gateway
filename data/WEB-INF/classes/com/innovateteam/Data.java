...
public class Data extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            fileName = reqPath.substring(reqPath.lastIndexOf('/') + 1);
         
            if (!authenticated && reqPath.startsWith("RESTRICTED")) {
                String redireLogin = request.getScheme() + "://" + request.getServerName() + "/sso/login";
                String reirecSSOback = "?sso_redirect=";
                response.sendRedirect(redireLogin + reirecSSOback + request.getRequestURI());
                log.info("redirect to : " + redireLogin + reirecSSOback + request.getRequestURI());
                /*if (isFile) {
                    ...
                    response.sendRedirect("https://edg.epa.gov/metadata/catalog/identity/login.page");
...
//                      getDir("http://192.168.49.211/ftpdata/" + reqPath,response,suppressHeader);
   ...
   /**
    ...
    private void getDir(String dirUrl, HttpServletResponse resp, String suppressHeader) {
        //definitions of the strings used for very large file
        String jsonFilePrefix = "EPADataCommons_";
        String filenameField = "filename";
        String ftpField = "ftpurl";
        String urlField = "url";
        ...
        try {
            ...
            //log.info("bji xmlToString before modify: "+xml.toString());
            //start of modifying xml
            ...
            for(int i=0; i<entries.getLength();i++){
                elementFile = (Element) entries.item(i);
                if (elementFile.getAttribute("type").equalsIgnoreCase("file"))
                {
                    Element fileLocationOnA = (Element)elementFile.getElementsByTagName("A").item(0);
                    String fileLocation = fileLocationOnA.getAttribute("HREF");

                    String jsonFileName = fileLocation.substring(fileLocation.lastIndexOf('/') + 1);

                    if (jsonFileName.startsWith(jsonFilePrefix))
                    {

                        URL jsonFile = new URL( dataLoc + fileLocation.substring(6)  );
                        BufferedReader in = new BufferedReader(new InputStreamReader(jsonFile.openStream()));

                        String inputLine;
                        String jsonContent = "";
                        while ((inputLine = in.readLine()) != null){
                            jsonContent = jsonContent+ inputLine;
                        }
                        in.close();

                        JSONObject obj = new JSONObject(jsonContent);
                        
                        Node genderNode = elementFile.getElementsByTagName("A").item(0);
                        elementFile.removeChild(genderNode);
                        genderNode = elementFile.getElementsByTagName("date").item(0);
                        elementFile.removeChild(genderNode);                        
                        genderNode = elementFile.getElementsByTagName("time").item(0);
                        elementFile.removeChild(genderNode);                        
                        genderNode = elementFile.getElementsByTagName("fileSize").item(0);
                        elementFile.removeChild(genderNode);  
                        
                        Element newFileNameEle = null;
                        
                        if (obj.has(filenameField)){
                            newFileNameEle = doc.createElement("A");
                            newFileNameEle.appendChild(doc.createTextNode(obj.getString(filenameField)));
                            elementFile.appendChild(newFileNameEle); 
                        }
                        
                        if (obj.has(urlField)){
                            newFileNameEle = doc.createElement("A1");
                            newFileNameEle.appendChild(doc.createTextNode("http"));
                            elementFile.appendChild(newFileNameEle);                            
                            
                            newFileNameEle = doc.createElement("HREF1");
                            newFileNameEle.appendChild(doc.createTextNode(obj.getString(urlField)));
                            elementFile.appendChild(newFileNameEle);  
                        }
                        
                        if (obj.has(ftpField)){
                            newFileNameEle = doc.createElement("A2");
                            newFileNameEle.appendChild(doc.createTextNode("ftp"));
                            elementFile.appendChild(newFileNameEle);    
                            
                            newFileNameEle = doc.createElement("HREF2");
                            newFileNameEle.appendChild(doc.createTextNode(obj.getString(ftpField)));
                            elementFile.appendChild(newFileNameEle);       
                        }

                        if (obj.has(dateField)){
                            newFileNameEle = doc.createElement("date");
                            newFileNameEle.appendChild(doc.createTextNode(obj.getString(dateField)));
                            elementFile.appendChild(newFileNameEle);  
                        }
                        if (obj.has(timeField)){
                            newFileNameEle = doc.createElement("time");
                            newFileNameEle.appendChild(doc.createTextNode(obj.getString(timeField)));
                            elementFile.appendChild(newFileNameEle);        
                        }
                        if (obj.has(sizeField)){
                            newFileNameEle = doc.createElement("fileSize");
                            newFileNameEle.appendChild(doc.createTextNode(obj.getString(sizeField)));
                            elementFile.appendChild(newFileNameEle);  
                        }
                        
                        newFileNameEle = doc.createElement("style");
                        newFileNameEle.appendChild(doc.createTextNode("text-decoration:underline"));
                        elementFile.appendChild(newFileNameEle);                        

                        newFileNameEle = doc.createElement("space");
                        newFileNameEle.appendChild(doc.createTextNode("       "));
                        elementFile.appendChild(newFileNameEle);                         
                        
                        elementFile.setAttribute("type", "largefile");
                    }
            ...
            //log.info("bji writer: "+  writer.toString());
            out.print(xform(writer.toString(),"/xformDir.xsl"));
            //end of modifying xml
            //out.print(xform(xml.toString(),"/xformDir.xsl"));
